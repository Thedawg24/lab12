...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0) { throw new TreeException("Colon does not exist"); } // with a useful message
		String parent = line.substring(0, colonIndex);	//The substring of line that starts at char #0 and ends just before colonIndex					
		String childrenString = line.substring(colonIndex + 1); //The substring of line that starts just after colonIndex and goes through the end of the line.
		String[] childrenArray = childrenString.split(","); // Call childrenString.split(). The result will be an array of strings, with the separating commas thrown away.
		...
		if (root == null) { parentNode = root = new TreeNode(parent); }
			...
			if (parentNode == null) { throw new TreeException("Node does not exist"); }//If the parent node wasn't found, there must have been something wrong in the data file. Throw an exception.
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null) { throw new TreeException("A node with the name \"" + name1 + "\" does not exist."); }
		TreeNode node2 = root.getNodeWithName(name2);
		if (node2 == null) { throw new TreeException("A node with the name \"" + name2 + "\" does not exist."); }
